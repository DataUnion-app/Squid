...
export const BASE_URL = "https://crab.dev.dataunion.app";
...
class API {
  ...
  call = (path, method, data, headers, isPure) => {
    ...
    return fetch(`${BASE_URL}/${path}`, param)
      .then((response) => {
        if (isPure) {
          return response;
        }
  ...
  thumbnail = async (id) => {
    return this.call(
      `api/v1/image/thumbnail?image_id=${id}`,
      "GET",
      null,
      null,
      true
  ...
  removeDataSet = async ({ index }) => {
    return this.call(`api/v1/entity-lists/?id=${index}`, "DELETE")
      .then((res) => {
        console.log(res);
        return true;
      })
  ...
  saveData = async (
    ...
    return this.call("api/v1/entity-lists/update", "POST", {
      id,
      visibility,
      entity_ids,
      description,
  ...
  changeDataSetName = async ({ oldname, newname }) => {
  ...
  removeSelectDatas = async ({ name, images }) => {
    const datas = JSON.parse(localStorage.getItem("datas")) || [];
    const a_index = datas.findIndex((item) => item.name === name);
    let i;
    for (i = 0; i < images.length; i++) {
      const index = datas[a_index].photos.findIndex(
        (item) => item === images[i].hash
      );
      datas[a_index].photos.splice(index, 1);
    }
    localStorage.setItem("datas", JSON.stringify(datas));
  ...
  fetchDatas = async (page = 1) => {
    return this.call(
      `api/v1/entity-lists/own?page=${page}&entity_type=image`,
      "GET"
    )
      .then((response) => {
  ...
  tags = (start_date, end_date) => {
    return this.call(
      `/api/v1/stats/overall-tags?start_date=${start_date}&end_date=${end_date}`,
      "GET"
    )
      .then((response) => {
        const list_of_removed_tags = [
          "women",
          "woman",
          "MAN",
          "WOMEN",
